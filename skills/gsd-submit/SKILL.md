...
| **위 어디에도 안 맞는 것** | **기타 — 프로젝트 주제를 직접 써서 넣는다** |

여러 개에 해당하면 복수로 넣어도 된다(복수선택 질문이다). 문구는 기수마다 조금씩 다를 수 있어
정확히 일치하지 않아도 알아서 맞춰 넣는다.

**맞는 보기가 하나도 없으면 `기타` 로 가되, 빈 `기타` 를 내지 않는다.**
`categories` 에 넣은 문구가 어느 보기와도 안 맞으면 그 문구가 그대로 **기타의 주제 텍스트**가 된다.
그러니 억지로 비슷한 보기를 고르지 말고, **프로젝트에 맞는 주제를 짧은 명사구로 직접 써서 넣는다.**

```json
"categories": ["반려동물 산책 기록 앱"]
```

`반려동물 산책 기록 앱` 은 보기에 없으므로 기타 주제로 들어간다.
`categories` 를 아예 비우면 제출물 주제(`title`)가 기타 주제로 쓰인다.

> 밀그램 UI도 기타를 고르면 텍스트만 보낸다(`{value, otherText}` 를 제출 직전에 `otherText` 로 바꾼다).
> 그래서 이렇게 넣으면 **사람이 손으로 낸 것과 완전히 같은 모양**으로 저장된다.
